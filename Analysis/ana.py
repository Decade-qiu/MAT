...
def get_rules():
    for i in rule_scale:
        with open(os.path.join(parent_path, f"rules_{i}k"), "r") as f:
            lines = f.readlines()
            for line in lines:
                tp = line.strip().split()
...
def show_ip_mask_distribution():
    for i in rule_field[:2]:
        ...
        plt.xlabel(f"rule数量")
        rules = global_total_rules[rule_scale[0]]
        ...
        for rule in rules:
            ...
            mask_cnt[mask] += 1
            mask_set.add(mask)
        print(field_name, sorted(mask_set))
...
def show_src_dst_tuple_mask_3D_distribution():
    ...
    for line in global_total_rules[scale]:
        src_plen = int(get_prefix(line[1]))
        dst_plen = int(get_prefix(line[3]))
