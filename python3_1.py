...
def print_info(tree: Node) -> None:
    ...
    write(f"Tree\t"+"  ".join(f'{"L"+str(i)+": "+str(v):9}' for i, v in enumerate(map(str, per_layer_rule))), 0, file)
    ...
    assert tcams_rule+rule_total-virtual_node == len(RULES_SET), f"total rules not equal! {tcams_rule+rule_total} {virtual_node} {len(RULES_SET)}"
